    ...
    install.addListener({
      ...
      onDownloadFailed: function(install)
      {
        ...
        if (install.error == AddonManager.ERROR_SIGNEDSTATE_REQUIRED)
        {
          ...
          notification.display("Add-on download failed (error code " + install.error + ").");
      },
      onInstallFailed: function(install)
      {
        install.removeListener(this);
        notification.display("Add-on installation failed (error code " + install.error + ").");
      }
    });
    install.install();
...
function installTemporaryAddon(data)
{
  ...
  {
    Cu.reportError(e);
    notification.display("Instalation of temporary add-on failed: " + e);
